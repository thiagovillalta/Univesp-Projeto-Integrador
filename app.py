...
# 📌 Rota para receber RFID do ESP8266
...
def verificar_rfid():
    ...
    if usuario:
        ...
        db.session.commit()
        return jsonify({"status": "success", "message": f"Acesso permitido para {usuario.name, usuario.id}"}), 200
    else:
        return jsonify({"status": "error", "message": "Acesso negado - ID não registrada"}), 403
...
def add_user():
    ...
    if form.validate_on_submit():
        ...
        if user is None:
            ...
            rfid_tag=form.rfid_tag.data, password_hash=hashed_pw)
            db.session.add(user)
            db.session.commit()
        name = form.name.data
        form.name.data = ''
